...
每天早上 08:00–12:00 之间跑一次（`agent_plan_enabled` **默认开启**——计划只产出提议，
跑了的代价是一个可以无视的队列，不跑的代价是一天没人规划；随时可关），
也可以随时手动点「出今日计划」。
...
**日报**：每天 18:00 后推一条——发了多少、收到几条回复、**几家在等你报价**、
执行了几条、还有几条等你定。三条通道按序尝试，一条通就算送到：

| 通道 | 配置 | 说明 |
|---|---|---|
| WhatsApp | `backend/report_whatsapp.txt` 写自己的号码 | **首选**：本机已有登录会话，不引入第三方，也不让任何外部服务读到客户名和金额 |
| 飞书 | `backend/lark_webhook.txt` | 自定义机器人 webhook |
| 企业微信 | `backend/wecom_webhook.txt` | 群机器人 webhook |

个人微信没有官方接口。Server酱一类转发服务能送到微信，但日报正文含客户名与金额，
要过一遍别人的服务器，因此不提供。

一条都没配就不自动推，只在 Agent 页显示——否则「今天已发日报」的标记会被白白用掉。
