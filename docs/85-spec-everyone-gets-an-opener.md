...
## R3：通用版开场白，说一句书本身能背书的话

补不出类别的，落到通用版。Allen：

> 通用版可以说我看到贵司有做 LED DISPLAY 相关的业务等等

| | |
|---|---|
| 英语（韩国以外） | `Saw that your company works with LED displays.` |
| 韩语（只给韩国） | `귀사에서 LED 디스플레이 관련 업무를 하고 계신 것을 봤습니다.` |

**这一句确实对客户做了声明**，而这份文件里其他每一句都刻意不做。它站得住，
因为依据是这本书本身：一家公司出现在这里，要么是发现阶段判定它是 LED 公司，
要么是 Allen 自己已经和它往来过 —— 落到通用版的 103 家里有 15 家是小满导入的，
往来记录是 P1.86 COB 报价单和产品目录。

[docs/45](45-spec-no-claim-without-a-source.md) 要的是**来源**，不是沉默。
「你们做 LED 显示屏」是这一行每一家都支撑得住的唯一一句话 —— 再具体一点
（租赁、户外、某个城市）就没有来源了，所以一个字都不能再多。

公司名按 Allen 的规矩用「贵司」代替：
