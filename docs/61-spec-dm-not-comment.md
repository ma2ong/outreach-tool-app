...
用真实页面探测（`app/fb_probe.py`，只读不发）确认了两件事：

```
[1] aria-label = 以 Allen Ma 的身份评论        ← 评论框
[2] aria-label = 发消息给Jaws Audio            ← 私信框
[3] aria-label = 发消息给PRI Productions       ← 另一家公司的私信框
```

第一件：这些框的祖先节点全是**没有 role 的普通 DIV**，所以"在对话框容器里"这条判据在
Facebook 上根本不成立。能分辨的只有标签本身。

第二件更要紧：**页面上会同时开着好几家公司的聊天窗**。取"最后一个私信框"就会把这家客户的
开发信发给另一家公司——那不是发错渠道，是发错人。

所以私信框的标签里必须出现我们要发的那个主页名，比对时忽略大小写和空格
（`jawsaudio` ⊂ `发消息给Jaws Audio`）。匹配不到、或者匹配到多个，一律中止。
