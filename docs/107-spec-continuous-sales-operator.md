...
outer border box. The terminal channel-status column must reserve enough content width
for Linux CJK fallback glyph metrics, so its non-wrapping header cannot paint beyond the
table's right edge.
