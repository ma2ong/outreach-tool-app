...
export interface Deliverability { days: number; sends: number; bounced: number; bounce_rate: number; danger: boolean; blind: boolean; sync_broken: boolean; unmeasured: number }
...
export async function setMailboxPassword(id: number, password: string): Promise<void> {
  const r = await fetch(`/api/mailboxes/${id}/password`, {
