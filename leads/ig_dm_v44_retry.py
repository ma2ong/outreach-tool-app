"""
IG DM v44 retry — 4 accounts that failed due to focus/input issue
Fix: JS el.focus() + el.click() immediately before type_text (no 1.5s delay)
     + retry up to 3 times with focus verification
"""
...
(function() {
  ...
  return JSON.stringify({
    ok: true,
    tag: el.tagName,
    ph: el.getAttribute("aria-placeholder") || el.placeholder || "",
    isFocused: active === el,
...
def get_input_len(tid):
    try:
        return int(eval_js(tid, GET_INPUT_LEN_JS) or "0")
    except Exception:
        return 0


def focus_and_type(tid, message, max_attempts=3):
    """Focus contenteditable via JS then immediately type. Retry up to max_attempts."""
    for attempt in range(1, max_attempts + 1):
        focus_r = json.loads(eval_js(tid, FOCUS_AND_CHECK_JS) or "{}")
        print(f"  focus({attempt}): {focus_r}", flush=True)
        if not focus_r.get("ok"):
            print(f"  ✗ contenteditable 未找到", flush=True)
            return False

        # Minimal delay — just enough for focus event to propagate
        time.sleep(0.3)
        ...
        print(f"  ↻ 输入为空，重试 {attempt}/{max_attempts}...", flush=True)
        time.sleep(2)
...
""")
    ...
    # Wait for DM panel to load — poll up to 30s
    FIND_INPUT_JS = r"""
(function() {
  var els = Array.from(document.querySelectorAll("[contenteditable=true],[role=textbox]"));
  var el = els.find(e => (e.getAttribute("aria-placeholder") || e.placeholder || "").length > 0);
...
"""
    ...
    for wait_i in range(30):
        ...
        if inp_check.startswith("found:"):
            print(f"  input appeared after {wait_i+1}s: {inp_check}", flush=True)
            break
    else:
        # Debug: list all buttons on page
        all_btns = eval_js(tid, "Array.from(document.querySelectorAll('[role=button],button')).map(b=>b.innerText.trim()).filter(t=>t).slice(0,10).join('|')")
        print(f"  input check: {inp_check} | btns: {all_btns}", flush=True)
        close_tab(tid)
        return False

    # Focus + type with retries
    ok = focus_and_type(tid, message)
    if not ok:
        print(f"  ✗ 输入失败，尝试截图...", flush=True)
        cdp(f"/screenshot?target={tid}&file=C:/tmp/ig_dm_fail_{username}.png", timeout=10)
        close_tab(tid)
        return False
    ...
    # Send
    send_r = json.loads(eval_js(tid, SEND_BTN_JS) or "{}")
    if not send_r.get("ok"):
        print(f"  ✗ 发送按钮未找到: {send_r}", flush=True)
        close_tab(tid)
        return False
...
def main():
    ...
    # Send to all targets (skip already messaged)
    queue = []
    for u in TARGET_USERNAMES:
        p = prospects.get(u, {})
        if p.get("status") == "messaged":
            print(f"  skip @{u} — already messaged", flush=True)
        else:
            queue.append(u)

    print(f"=== IG DM v44 RETRY | {len(queue)} accounts | {TODAY} ===\n", flush=True)
