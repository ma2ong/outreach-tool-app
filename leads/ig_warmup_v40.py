"""
IG Warmup v40 — v40 USA batch (8 accounts) + @insaneimpact pending from v39
Insane Impact (@insaneimpact) - Chicago [v39 pending]
Rocket Productions USA (@rocketprousa) - Atlanta
Meyer Pro Inc (@meyerproinc) - Portland
Limitless Lights and Sound (@limitlesslightsandsound) - Houston
Illuminated Mobile (@illuminatedmobile) - Chicago
C West Entertainment (@cwestent) - Phoenix
Sifi Entertainment (@sifientertainment) - Atlanta
Master Sound Pro (@mastersoundpro) - Fort Lauderdale
"""
...
(function() {
  ...
  var fb = btns.find(b => { var t = b.innerText.trim(); return t === "关注" || t === "Follow"; });
  if (!fb) return JSON.stringify({ok: false, reason: "follow button not found (may already follow)"});
  fb.click();
  return JSON.stringify({ok: true});
})()
""")
...
def main():
    ...
    queue = [u for u in TARGET_USERNAMES if u in prospects and not prospects[u].get("warmup_done")]
    print(f"=== IG Warmup v40 | {len(queue)} accounts | {TODAY} ===\n", flush=True)
    done = 0
    ...
    print(f"=== Done: {done}/{len(queue)} accounts warmed up ===")
    print(f"Next: run ig_dm_v40.py")
